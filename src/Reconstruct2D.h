...
const int MODE_NONE = 0, MODE_BLEND = 1, MODE_DENOISE = 2;
...
private:
	...
	float minNoise, runtimeC, runtimeD, oldAvgSD, newAvgSD, oldAngleSum, newAngleSum;
...
public:
	...
	Reconstruct2D(const vector<Point> &p_points, float p_minNoise, int mode);
	...
	vector<float> getEstimatedNoiseExtents();
